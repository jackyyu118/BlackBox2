...
ElfImg::ElfImg(std::string_view base_name) : elf(base_name) {
    ...
    if (fd < 0) {
        ALOGE("failed to open %s", elf.data());
        return;
    }

    size = lseek(fd, 0, SEEK_END);
    if (size <= 0) {
        ALOGE("lseek() failed for %s", elf.data());
    }
...
ElfW(Addr) ElfImg::LinearLookupShort(std::string_view name) const {
    if (symtabs_.empty()) {
        symtabs_.reserve(symtab_count);
        if (symtab_start != nullptr && symstr_offset_for_symtab != 0) {
            for (ElfW(Off) i = 0; i < symtab_count; i++) {
    ...
    for(auto kv : symtabs_){
        auto symname = kv.first;
        if(symname.find(name) != std::string_view::npos){
            ALOGE("found symbol %s by short %s", symname.data(),name.data());
            return kv.second->st_value;
        }
    }
    return 0;
//    if (auto i = symtabs_.find(name); i != symtabs_.end()) {
...
ElfImg::getSymbOffset(std::string_view name, uint32_t gnu_hash, uint32_t elf_hash) const {
    if (auto offset = GnuLookup(name, gnu_hash); offset > 0) {
        ALOGD("found %s %p in %s in dynsym by gnuhash", name.data(),
             reinterpret_cast<void *>(offset), elf.data());
        return offset;
    } else if (offset = ElfLookup(name, elf_hash); offset > 0) {
        ALOGD("found %s %p in %s in dynsym by elfhash", name.data(),
             reinterpret_cast<void *>(offset), elf.data());
        return offset;
    } else if (offset = LinearLookup(name); offset > 0) {
        ALOGD("found %s %p in %s in symtab by linear lookup", name.data(),
             reinterpret_cast<void *>(offset), elf.data());
        return offset;
    } else if (offset = LinearLookupShort(name); offset > 0) {
        ALOGD("found %s %p in %s in symtab by linear lookup short", name.data(),
              reinterpret_cast<void *>(offset), elf.data());
        return offset;
    } else {
        return 0;
...
bool ElfImg::findModuleBase() {
    ...
    if (!found) {
        ALOGE("failed to read load address for %s", elf.data());
        fclose(maps);
        return false;
    }
    ...
    ALOGD("get module base %s: %lx", elf.data(), load_addr);
